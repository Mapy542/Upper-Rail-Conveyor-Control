...
// 1 or AccelStepper::DRIVER means a stepper driver (with Step and Direction pins) enablePin is not library controlled.
...
void alarmMan()
{
  Serial.print("alarm state: " + alarmstate + "alarm freq: " + alarmfreq + "alarm set time: " + alarmsettime);
  switch (alarmstate)
  {
  case 0:
    noTone(alarm);
  ...
  case 1:
    if (alarmfreq > 1000)
    { // if we go straight from error alarm to low alarm then reset alarm cycle
      alarmfreq = 0;
    }
    if (alarmfreq == 0)
    {
    ...
    if (alarmfreq == 800 && (millis() - alarmsettime) > 500)
    { // after 500 milisecs or when available set new tone freq
      tone(alarm, 1000);
      alarmsettime = millis();
      alarmfreq = 1000;
    }
    if (alarmfreq == 1000 && (millis() - alarmsettime) > 500)
    { // after 500 milisecs or when available set new tone freq
      tone(alarm, 800);
      alarmsettime = millis();
      alarmfreq = 800;
  ...
  case 2:
    ...
    if (alarmfreq == 1500 && (millis() - alarmsettime) > 500)
    { // after 500 milisecs or when available set new tone freq
      tone(alarm, 2000);
      alarmsettime = millis();
      alarmfreq = 2000;
    }
    if (alarmfreq == 2000 && (millis() - alarmsettime) > 500)
    { // after 500 milisecs or when available set new tone freq
      tone(alarm, 1500);
      alarmsettime = millis();
      alarmfreq = 1500;
...
void curtainHalt()
{
  ...
  {
    Serial.print(" Curtain Made ");
  }
}

void loop()
{
  Serial.print(" state: " + state + " transferstate: " + transferstate + " movestate: " + movestate);
  // control
  switch (state)
  {
  case 0:
  ...
  case 1:
    ...
    if (!digitalRead(robotPermit) && transferstate == 0)
    { // detect rising edge of robot permit for takeout
      transferstate = 1;
    }
    if (digitalRead(robotPermit) && transferstate == 1)
    { // detect falling edge of robot permit
      transferstate = 2;
      state = 2;
    }
  ...
  case 2:
    ...
    { // cycle forward
      ...
      if (movestate == 0)
      {
        ...
        if (digitalRead(leftEye) && digitalRead(rightEye))
        { // if we see parts in both then we good
          movestate = 1;
        }
        else
        { // stuck or missing part
          stepper.stop();
          state = 3;
          break;
        }
      ...
      if (movestate == 1)
      { // move forward untill both eyes go empty
        stepper.runSpeed();
        curtainHalt();
        if (!digitalRead(leftEye) && !digitalRead(rightEye))
        { // both go empty
      ...
      if (movestate == 2)
      { // finsihed reset states and break case
        movestate = 0;
        transferstate = 0;
        state = 1;
        break;
    ...
    { // rehome
      digitalWrite(condition, LOW);
      if (digitalRead(leftEye) && digitalRead(rightEye))
      { // if both eyes have parts in sight then load like normal
        stepper.setSpeed(50);
        do
        {
          stepper.runSpeed();
      ...
      else
      {
        ...
        stepper.stop();
        if (digitalRead(leftEye) && digitalRead(rightEye))
        { // there was rails ready to move them forward
        }
        else if ((digitalRead(leftEye) || digitalRead(rightEye)) && !(digitalRead(leftEye) && digitalRead(rightEye)))
        { // only one detected so bad things
          state = 3;
          stepper.stop();
        }
        else
        { // there never was ready rail so move the next ones forward
          movestate = 0;
          do
          {
            stepper.setSpeed(50);
